...
Machine learning pervades most modeling and decision-making tools of modern society: scientists use the wealth of stored medical records to understand the underlying causes of diseases, intelligent personal assistants learn the transportation patterns of an area and provide context-aware optimal route recommendations, and driving-intelligence systems are capable to navigate self-driving cars in complex, never-seen-before environments. 

From the statistical point of view, Bayesian modeling offers a unifying framework where experts and practitioners alike can leverage domain-specific knowledge, learn from new observations, share statistical power across components of hierarchical models, and take advantage of predictions which can account for model uncertainty. Having access to more data is invaluable for statistical models, as it allows a better understanding of the process that gives rise to them. 

At the same time, handling massive-scale datasets in machine learning instigates a number of computational and societal challenges. First, apart from basic statistical settings, peforming inference, \ie~computing expectations under posterior distributions in the light of new observations, does not scale for large datasets. Hence, approximate inference methods should be followed instead. Apart from addressing scalability, modern approximate inference methods should be also able to give guarantees of convergence to the true posterior distribution, and work successfully in high-dimensions, where many of modern large-scale data live (e.g. genes, or social networks).

Secondly, a large fraction of modern massive-scale machine learning applications involves observations stemming from privacy-sensitive data domains, for example health or behavioural studies. The sensitive information content of this sources raise to data contributors the requirement that inference methods satisfy formal guarantees of statistical privacy.

Thirdly, real-world big data are often highly heterogeneous, containt outliers, or might be subject to data poisoning. The afore-mentioned phenomena are often expressed as patterns which cannot be captured by standard statistical assumptions of the model. As a result, standard Bayesian inference techniques which do not downweight the contribution of outlying datapoints can often be misguided by them and offer unreliable posteriors. 
