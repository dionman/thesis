...
%; an overview of our approach is depicted in Figure~\ref{fig:methodology}.

%\begin{figure*}[!t]
%	\centering
%	\includegraphics[totalheight=8cm, width=0.8\textwidth]{fig/methodology_diagram.eps}
%	\caption{An overview of our approach. All mobility information starts as a sequence of pseudonymous locations. In the first step, we assume the attacker has access to some mobility information for one or more individuals, and this is parsed into mobility networks, one per individual. Then, the mobility networks are projected into a feature space, where we compute their pairwise similarities. In the second step, the attacker attempts to deanonymize unlabelled mobility information; this can be done by projecting the mobility information into the same feature space as before and minimising the pairwise distance in the feature space to identify the unlabelled mobility information.
%	}
...
We convert a location sequence for each user into a mobility network (Section \ref{sec:mobility-networks}). %, the details of which are described later in Section~\ref{sec:mobility-net-construct}.
We then extract feature representations of these networks and embed them into a vector space.
Finally, in the vector space, we can define pairwise distances between the network embeddings (Section~\ref{sec:graph-kernels}) and use them in a deanonymization scenario (Section~\ref{sec:deanon-leakage}).  %as described in Section~\ref{sec:deanon-leakage}.
...
\ndefn[(\textbf{$k-$anonymity})]{
	A microdata release of statistics containing separate entries for a number of individuals in the population satisfies the $k-$anonymity property if the information for each individual contained in the release is indistinguishable from at least $k-1$ other individuals whose information also appears in the release.
}
...
The alternative is that a sequences of the states are better modelled by higher-order Markov chains, namely that transitions depend on the current state and one or more previously visited states.
...
A \textbf{state connectivity network} for $ u $ is an unweighted directed graph $ C^{u}=\big(V^{u}, E^{u}\big) $. Nodes $ v_i \in V^{u} $ correspond to states visited by the user throughout the observation period.  An edge $ e_{ij} = \big(v_i^u, v_j^u\big) \in E^{u} $ represents the information that $ u $ had at least one recorded transition from $ v_i^u $ to $ v_j^u $.
}

\ndefn[\textbf{Mobility Network}]{ 
A \textbf{mobility network} for $ u $ is a weighted and directed
graph $G^{u}=\big(V^{u}, E^{u}, W^{u}\big)  \in \mathcal{G}$, with the same topology as the state connectivity network and additionally an edge weight function $ W^{u} : E^{u} \rightarrow \mathbb{R}^{+}$. The weight function assigns a frequency $ w_{ij}^u $ to each edge $ e_{ij}^u $, which corresponds to the number of transitions from $ v_i^u  $ to $ v_j^u $ recorded throughout the observation period.
}
...
We wish to compute the similarity between two graphs $ G, G' \in \mathcal{G} $. Kernel functions~\cite{Vishwanathan2010}, or \emph{kernels}, are symmetric positive semidefinite functions, where  $ K(G, G') : \mathcal{G} \times \mathcal{G} \rightarrow \mathcal{R}^{+} $, meaning
...
In order to ensure the result from the kernel lies in the range from $-1$ to $1$ inclusive, we apply \emph{cosine normalization} as follows:
\begin{equation}
...
\begin{equation}
\phi(G) = [\#\big(s_1 \in G\big), \#\big(s_2 \in G\big), ...]^T
\end{equation}

 with dimension $ |\mathcal{S}| $  and kernel

\begin{equation}
K(G, G') = \sum_{s \in \mathcal{S}} \#\big(s \in G\big) \#\big(s \in G'\big).
\end{equation}
...
Shervashidze et al.\ proposed an efficient method~\cite{Shervashidze2011} to construct a graph kernel utilizing the \mbox{Weisfeiler-Lehman} (\emph{WL}) test of isomorphism~\cite{weisfeiler1968reduction}.
...
\]
where $ \phi_h(G) $ and $ \phi_h(G') $ are the vectors of labels extracted after running $ h $ steps of the computation (Figure~\ref{fig:feat_G}). They consist of $ h $ blocks, where the $i-$th component of the $j-$th block corresponds to the frequency of label $ i $ at the $j-$th iteration of the computation.
...
\begin{figure*}[!h]
	...
	\begin{subfigure}[t]{.19\textwidth}
		\resizebox{0.6\textwidth}{0.08\textheight}{%
			...
			\tikzstyle{state1}=[circle,draw=black,text=red!50, font=\small]
			\tikzstyle{state2}=[circle,draw=black,text=blue!50!, font=\small]
			\tikzstyle{state3}=[circle,draw=black, text=black!50,  font=\small]
			...
			\tikzstyle{state9}=[circle, draw=black,text=orange!80,  maximum size=0.5cm,font=\small]
			\node[state3] 		 (A0)                    {$3$};
			...
			\end{tikzpicture}}
		\caption{\sffamily{$G$}} \label{fig:G}
	\end{subfigure}
	\begin{subfigure}[t]{.19\textwidth}
		\resizebox{0.6\textwidth}{0.08\textheight}{%
	...
	\begin{subfigure}[t]{.19\textwidth}
		\resizebox{0.6\textwidth}{0.08\textheight}{%
			\begin{tikzpicture}[shorten >=1pt, thick, auto,node distance=1.5cm,font=\small]
			\tikzstyle{state1}=[circle,draw=black,text=red!50,scale=0.8, font=\small]
			\tikzstyle{state2}=[circle,draw=black,text=blue!50!,scale=0.8, font=\small]
			\tikzstyle{state3}=[circle,draw=black, text=black!50,  scale=0.8,font=\small]
			...
			\tikzstyle{state9}=[circle, draw=black,text=orange!80,  maximum size=0.5cm,scale=0.8,font=\small]
			\node[state3] 		 (A0)                    {$3,233$};
			...
			\draw (C0) edge[thick]  (F0);
			\end{tikzpicture}}
		\caption{ \sffamily{$G$ with multiset node attributes}} \label{fig:Gmult}
	\end{subfigure}
	\begin{subfigure}[t]{.19\textwidth}
		\resizebox{0.6\textwidth}{0.08\textheight}{%
			\begin{tikzpicture}[shorten >=1pt, thick, auto,node distance=1.5cm]
			...
			\draw (D1) edge[thick]  (F1);
			\end{tikzpicture}}
		\caption{ \sffamily{$G'$ with multiset node attributes}} \label{fig:G'mult}
	\end{subfigure}
	\begin{subfigure}[t]{.19\textwidth}
		\begin{tikzpicture}[->, thick, auto,node distance=0.005cm,every node/.style={inner sep=0,outer sep=0}]
		\tikzstyle{state1}=[draw=white,text=red!50, font=\small]
		\tikzstyle{state2}=[draw=white,text=blue!50!, font=\small]
		\tikzstyle{state3}=[draw=white, text=black!50,  font=\small]
		...
		\tikzstyle{state9}=[ draw=white,text=orange!80,  font=\small]
		...
		\end{scope}
		\end{tikzpicture}
		\caption{{Label Compression}} \label{fig:mapping}
	\end{subfigure}
	...
	\begin{subfigure}[t]{.2\textwidth}
		\resizebox{0.6\textwidth}{0.08\textheight}{%
			...
			\tikzstyle{state9}=[circle, draw=black,text=orange!80,  font=\small]
			\node[state7] 		 (A0)                    {$7$};
...
\begin{figure}[!h]
	...
	\begin{subfigure}[]{0.4\textwidth}
		...
		\caption{{\textbf{user 1}: first half of the observation period}}
		\label{fig:evidence11}
	\end{subfigure}%
	\begin{subfigure}[]{0.4\textwidth}
		\centering
		\includegraphics[height=4cm]{\MyPath/fig/1_secondhalf_.pdf}
		\caption{{\textbf{user 1}: second half of the observation period}}
	...
	\begin{subfigure}[]{0.4\textwidth}
		...
		\caption{{\textbf{user 2}: first half of the observation period}}
		\label{fig:evidence21}
	\end{subfigure}
	\begin{subfigure}[]{0.4\textwidth}
		\centering
		\includegraphics[height=4cm]{\MyPath/fig/2_secondhalf_.pdf}
		\caption{{\textbf{user 2}: second half of the observation period}}
...
$ l_{G'} \in \mathcal{L}$.%
\footnote{Generally we can think of $ l_{G'} \in \mathcal{J} \supset \mathcal{L}$ and assign some fixed probability mass to the labels $ l_{G'} \in \mathcal{J} \setminus \mathcal{L}$.
...
\begin{equation}
	P\big(l_{G'}= l_{G_i}\big) = 1/|\mathcal{L}|, \mbox{ for every }  G_i \in \mathcal{G}_{\text{training}}.
\end{equation}
...
%
	...
	\begin{align}
	\begin{split}
	P\big(l_{G'} =l_{G_i}| \mathcal{G}_{\text{training}}, K\big)& =  \frac{f\big(K(G_i, G')\big)}{\displaystyle \sum_{j\in \mathcal{L}}  f\big(K(G_j, G')\big)},\\  & \text{for every }  G_i \in \mathcal{G}_{\text{training}}.
	\end{split}
...
\begin{equation} 
\begin{split}
PL\big(G';\mathcal{G}_{\text{training}}, K\big) = \frac{ P\big(l_{G'}= l_{G'_\text{true}}|\mathcal{G}_{\text{training}}, K\big)}{P\big(l_{G'}= l_{G'_\text{true}}\big)} -1
\end{split}
\label{eq:privloss}
\end{equation}
...
Under the above premise, the maximal privacy loss occurs when the presented test network is an identical copy of a training network of the same user which exists in the data of the adversary, i.e.\ $ G' \in \mathcal{G_\text{training}}$.
