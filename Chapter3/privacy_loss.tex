...
\begin{figure}[!t]
	\centering
	\includegraphics[width=\linewidth]{\MyPath/fig/kernels_evaluation.pdf}
...
\begin{figure*}[]
	...
	\begin{minipage}[b]{.45\textwidth}
		\includegraphics[width=\linewidth]{\MyPath/fig/rank_correct_.pdf}
	...
	\end{minipage}\qquad
	\begin{minipage}[b]{.45\textwidth}
		\includegraphics[width=\linewidth]{\MyPath/fig/posterior_privloss_.pdf}
...
\begin{itemize}
	...
	%\item \textbf{Removal of short-stay regions:} Reducing the dwell time required at a particular \emph{cid} in order to consider the \emph{cid} location as a node in the mobility graph does not affect the \mbox{identification task}.
	%This is because the nodes representing \emph{cid}s which are visited for short periods are subsequently removed when pruning the mobility network.
...
%\begin{figure}[!b]
%	\centering
%	\includegraphics[width=\linewidth]{fig/rank_correct.eps}
%	\caption{Boxplot of rank for the true labels of the population according to a Deep Shortest-Path kernel and to a random ordering.}
%	\label{fig:rank_correct}
%\end{figure}
...
%\begin{align}
%\begin{split}
%P\big(l_{G'} =l_{G_i}|\mathcal{G}_\text{training}, K_{DSP}\big) & \propto \frac{1}{rank\big(K_{DSP}(G_i, G')\big)},\\  & \mbox{for every }  G_i \in \mathcal{G}_\text{training}.
%\label{adv_rule}
%\end{split}
%\end{align}
