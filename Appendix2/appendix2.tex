...
\begin{proof}
For convenience let's focus on the case of Gaussian mean inference for the classic Bayesian posterior, where the objective can be written in closed form. Our argument will in principle carry over for arbitrary $\beta$s and statistical models. We recall from~\cref{eq:KL_from_coreset_post_to_exact} that
...
<0,
\]
where we have used the symmetry of the objective \wrt the observations and the fact that $d(S)=d(\mcX)=0$.
\end{proof}
