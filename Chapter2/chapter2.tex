...
In this section, we present robustness limitations of standard posterior on big data, and outline existing generalizations of the posterior that aim to robustify inference with respect to data mismatch. Adopting robustified posteriors as our target of coreset approximations, in~\cref{chap:chap5} we will successfully address scenarios of inference under model misspecification.
...
When datapoints are conditionally independent given $\theta$, the likelihood function gets factorized as $\pi(x|\theta) = \Pi_{n=1}^{N}\pi(x_n|\theta)$. An equivalent formulation of  the
...
As mentioned in~\cref{subsec:b-divergences}, a popular choice~\citep{futami18, knoblauch18} for enhancing robustness of inference is replacing the log-likelihood terms arising in~\cref{eq:zellner-rule} with the \emph{\bdiv~}, which yields the following posterior for $\theta$~\citep{ghosh16,knoblauch18}
\[
