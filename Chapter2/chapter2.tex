...
\ndefn[Differential Privacy]{
	Fix $\veps \geq 0$. A randomised mechanism $ \mcM: \mcX \rightarrow \mcY$  is $\veps$-differentially private if for every pair of  adjacent datasets $ x \approx x'$ and all events $ A \subseteq \mcY$, 	$\;\Pr[\mcM(x) \in A] \leq e^\veps \Pr[\mcM(x') \in A]$.
...
A weaker definition of DP allows that the guarantee of~\cref{def:b-dp-definition} gets broken with probability $\delta$. This corresponds to the notion of \emph{($\veps, \delta$)-approximate differential privacy}, and will be the privacy definition applied on our summarization scheme in~\cref{chap:chap4}.
