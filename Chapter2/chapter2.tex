...
\]
Markov Chain Monte Carlo~(MCMC), the workhorse of approximate Bayesian inference, is a framework consisting of methods that pursue the above idea efficiently. MCMC offers approximations to expectations \wrt intractable probability distributions via simulating an ergodic random walk which admits the (unnormalized) posterior distribution as its stationary distribution; this property makes MCMC methods theoretically appealing, as it endows the estimators with strong \emph{asymptotic exactness} guarantees. Differing on the way that the Monte Carlo chain is constructed, several methods of MCMC infrence have emerged, including the Metropolis-Hastings~\citep{andrieu03}, the Hamiltonian Monte-Carlo~\citep{neal11}, and the No-U-Turn-Sampler~(NUTS)~\citep{hoffman14}. NUTS will be used to perform inference in part of our experiments over~\cref{chap:chap4,chap:chap5}.

\subsection{Variational Inference}
\label{subsec:b-variational-inference}
The problem formulation underpinning all variational inference~(VI) methods~\citep{jordan99,blei17} is to find a member $q^*$ within a family of tractable probability distributions $Q$ that best approximates our posterior (typically in the KL-sense)
\[
\label{eq:bvi}
...
we note that in classical VI schemes expanding the KL divergence according to~\cref{eq:bkl-def} makes the log-evidence appear in the objective 
\[
\kl{q(\theta)}{\pi(\theta|x)} = \EE_{\theta \sim q(\theta)}\left[\log q(\theta)\right] - \EE_{z\sim q(\theta)}\left[\log \pi(x, \theta)\right] + \log \pi(\theta).
\]
Since this term is not a function of $q$, it can be subtracted and the problem is reduced to minimizing the negation of the first two terms instead, which is known as the \emph{evidence lower bound}~(ELBO).
\[
q^{*}(\theta; x) \defined \arg \underset{q\in Q}{\min} \left(-\text{ELBO}(q, x)\right), \qquad
\text{ELBO} \defined \EE_{q}\left[\log \pi(x, \theta)\right] - \EE_q \left[\log q(\theta)\right]
\]
The simplest family that can be used for VI is the \emph{mean-field variational family} which relies on the simplifying assumption of independence among the coordinates of $\theta$, \ie $q(\theta) \defined \Pi_{d=1}^{D}q_d(\theta_d)$.
Our VI scheme used approximation within the \emph{exponential family}, which can allow less restricted posterior. Additionally, they can circumvent the use of ELBO and be directly applied on the original variational formulation of~\cref{eq:bvi}, as MC computation of the gradient of the log-evidence can be done as per~\cref{prop:bgradZ}. 
...
Differential privacy~(DP)~\citep{dwork06} requires that if we perturb a database by a single data point the output of an algorithm should not differ much. To define distance between two databases $x, x'$ of size $ N $ we use the Hamming distance
\[
...
\ndefn[Differential Privacy]{
	Fix $\veps \geq 0$. A randomised algorithm
	$ \mcM: \mcX \rightarrow \mcY$  is $\veps$-differentially private if for every pair of 
	adjacent datasets $ x \approx x'$ and all events $ A \subseteq \mcY$, 
	$\;\Pr[\mcM(x) \in A] \leq e^\veps \Pr[\mcM(x') \in A]$.
	\label{def:b-dp-definition}
}

A weaker definition of DP allows that the guarantee of~\cref{def:b-dp-definition} gets broken with probability $\delta$. This definition is called \emph{($\veps, \delta$)-approximate differential privacy} and will be further discussed in~\cref{chap:chap4}.

DP is equipped with a suite of properties that facilitate reasoning about privacy guarantees over complicated analysis tasks on sensitive databases.
