...
\vspace{.2cm}
\par 
\textbf{Choice of the robustness hyperparameter value.} Selecting an appropriate value for $\beta$ when doing inference using power divergences has been treated as an instance of hyperparameter optimization. Prior knowledge on the expected subspace for the inliers can be leveraged in order to specify a reasonable value for the hyperparameter \emph{a priori}~(recall from~\cref{fig:influence} that $\beta$ controls the distance where the maximum of the concave data influence function is located). Earlier work in robust Bayesian inference has considered automating the selection of this value in the light of observations, using cross-validation~\citep{futami18}, or via performing on-line gradient descent on the expected predictive loss~\citep{knoblauch18}. In a similar vein, for the purposes of variational inference using other parameterised families of divergence functions, such as the $\alpha$- and $f$-divergence, recent approaches for adaptive learning of optimal hyperparameters have relied on meta-learning techniques~\citep{ruqizhang21}, on controlling the variance of Monte Carlo estimates used in variational inference~\citep{dilinwang18}.
