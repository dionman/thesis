...
for \sparsevi~is included in Appendix~\ref{app:experiments_appendix}. %Code for the presented experiments is available at~\href{https://github.com/dionman/psvi}{https://github.com/dionman/psvi}. 
...
(see Appendix~\ref{app:gaussian_experiment_appendix}),
\[ 
\begin{aligned}
...
\begin{figure*}[!t]
	...
	\end{subfigure}\hfill\qquad
	\begin{subfigure}[c]{.29\textwidth}
			\centerline{\scalebox{1}{\includegraphics[width=1.15\columnwidth]{\MyPath/figs/linregTT_KLDvssz.png}}}%
...
$101$-dimensional datapoints $(x_n,y_n)_{n=1}^{N}$ generated as follows: $(x_n)_{n=1}^N  \distiid \distNorm(0, I),\; (y_n)_{n=1}^N  \dist [1, x_n]^T\theta + \epsilon_n,\;  (\epsilon_n)_{n=1}^{N} \distiid \distNorm(0, \sigma^2)$, and aim to infer $\theta \in \reals^{101}$. We assume a prior $\theta \dist \distNorm(\mu_0,\sigma_0^2 I)$, where $ \mu_0, \sigma_0^2$ are the dataset empirical mean and second moment, and set the noise parameter $ \sigma$ to the variance of $(y_n)_{n=1}^{N}$. We apply stochastic optimization for \psvi~construction~(also see Appendix~\ref{app:linreg_model_appendix}). We use learning rates $\gamma_t = t^{-1}$ for \sparsevi, and $\gamma_t = 0.1t^{-1}$ for \psvi, $B=200$, $T=1000$, while selection step for \sparsevi~is carried out over the full dataset. \cref{fig:linreg_300} shows that Hilbert coresets cannot improve posterior approximation in this setting with 100 random projections (see Appendix~\ref{app:linreg_plots_appendix}), 
...
are deferred to Appendix~\ref{app:logreg_experiment_appendix}. 
%
...
\begin{figure*}[!t]
	...
	\begin{subfigure}[b]{.29\textwidth}
		\subcaption{(a)~\textsc{Transactions}~($d=50$)}
		\vspace*{-0.26cm}
	...
	\centering
	\begin{subfigure}[b]{.29\textwidth}
		\subcaption{(b)~\textsc{ChemReact100}~($d=100$)}
		\vspace*{-0.26cm}
