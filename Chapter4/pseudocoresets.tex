...
\bncor
Suppose the same setting with~\cref{prop:original_coreset_fails}. The optimal size-$M$ pseudocoreset $(u^\star,w^\start)$  defined on pseudodata $u_1, \ldots, u_M \in \mathbb{R}^d $ achieves ${\kl{\pi_{u^\star,w^\star}}{\pi}=0}$ for any size $M \geq 1$ and any data dimension $d$.
